...
class Brgmasuk extends BaseController {
    ...
    public function loadtable() {
        if (session()->get("logged_in")) {
            ...
            $idkapal = $this->request->uri->getSegment(4);
    ...
    private function simpan_detil() {
        ...
        if($cek > 0){
            ...
            $data = array(
                ...
                'verwendung' => $this->request->getVar('pn_sn'),
                'rak' => $this->request->getVar('pn_sn'),
                'idkapal' => $idkapal
            );
    ...
    public function gantidetil(){
        if(session()->get("logged_in")){
            ...
            $data = $this->model->getAllQR("SELECT a.idbrg_m_detil, a.idbarang, b.deskripsi, a.jumlah, a.satuan, b.pn_nsn, b.equipment_desc, c.idjenisbarang, b.rak "
                    . "FROM brg_masuk_detil a, barang b, brg_masuk c where a.idbrg_masuk = c.idbrg_masuk and a.idbarang = b.idbarang and a.idbrg_m_detil = '".$kode_detil."';");
    ...
    public function ajax_edit() {
        if(session()->get("logged_in")){
            ...
            if($update == 1){
                // cek nama barang klo belum ada masukkan dulu ke master barang
                $cek = $this->model->getAllQR("SELECT count(*) as jml FROM barang where deskripsi = '".$this->request->getVar('nm_barang')."' and idkapal = '".$this->request->getVar('kri')."';")->jml;
                if($cek > 0){
                    $kodebarang = $this->model->getAllQR("SELECT idbarang FROM barang where deskripsi = '".$this->request->getVar('nm_barang')."' and idkapal = '".$this->request->getVar('kri')."';")->idbarang;
                }else{
                    // masukkan ke master barang
                    $kodebarang = $this->model->autokode("B", "idbarang", "barang", 2, 7);
                    $data = array(
